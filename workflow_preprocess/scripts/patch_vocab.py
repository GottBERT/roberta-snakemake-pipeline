...
def main():
  ...
  if not path.isfile(path.join(args.dir, 'vocab.json.bu')):
    print('create backup of vocab.json')
    copyfile(path.join(args.dir, 'vocab.json'), path.join(args.dir, 'vocab.json.bu'))
  ...
  # read dict.txt
  dictionary = pd.read_csv(path.join(args.dict_file), delimiter=" ", header=None, names=['entry', 'count'])

  with open(path.join(args.dir, json_file)) as f_json_file:
  ...
  with open(path.join(args.dir, 'vocab.json'), 'w', encoding='utf-8') as f:
    json.dump(data, f)
