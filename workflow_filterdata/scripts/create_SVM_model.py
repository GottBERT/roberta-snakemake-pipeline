...
df_eval = pd.DataFrame(columns=['nu', 'accuracy', 'precision', 'recall', 'weighted_f1_score', 'mcc', 'report', 'model'])

# estimate best SVM given nu
for nu in list(map(lambda e: float(e), args.nu.split(","))):
    # create and train model
    model = svm.OneClassSVM(nu=nu)
    model.fit(X_train)
    ...
    list_row = [nu, accuracy, precision, recall, f1score, abs(mcc), report, model]
